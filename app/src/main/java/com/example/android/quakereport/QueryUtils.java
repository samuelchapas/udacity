...
public final class QueryUtils {
    ...
    public static List<earthquake> extractEarthquakes(String earthquakeJSON) {
        ...
        try {
            ...
            JSONArray arrayFeatures = reader.getJSONArray("features");

                    // looping through All Contacts
            for (int i = 0; i < arrayFeatures.length(); i++) {
                JSONObject c = arrayFeatures.getJSONObject(i);
                ...
                Log.v("query", place + date + magnitud);
                // adding contact to contact list
                earthquakes.add(new earthquake(place, date, magnitud, url));
            }
